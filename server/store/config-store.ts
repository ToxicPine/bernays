...
import type { BrowserConfig, ProxyConfig } from "$/browsers/types.ts";
  ...
  return {
      ...
      Effect.tryPromise({
        try: async () => {
          const rows = await sql`
            SELECT id, context, proxy_server, proxy_username, proxy_password
    ...
    upsert: (config) =>
      Effect.tryPromise({
        try: async () => {
          await sql`
            INSERT INTO browser_configs (id, context, proxy_server, proxy_username, proxy_password, updated_at)
            VALUES (
              ${config.id},
              ${config.context},
